...
export default function PostTwitForm(){
  ...
  const onSubmit= async(e:React.FormEvent<HTMLFormElement>)=>{
    ...
    if(!user||isLoading  || twit ===""||twit.length>180) return;
  ...
  return(
    <Form onSubmit={onSubmit}>
      ...
      rows={5}
      maxLength={180}
      onChange={onChange} value={twit} placeholder="What is happening?!"/>
      <AttachFileButton htmlFor="file">{file ? "Photo added ✅":"Add photo"}</AttachFileButton>
      <AttachFileInput onChange={onFileChange} type="file" id="file" accept="image/*"/>
      <SubmitBtn 
      type="submit" 
      value={isLoading? "Posting..." : "Post Twit"}/>
    </Form>
  )
}